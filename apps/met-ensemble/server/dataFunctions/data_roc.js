...
dataROC = function (plotParams, plotFunction) {
  ...
  for (curveIndex = 0; curveIndex < curvesLength; curveIndex++) {
    ...
    if (curveIndex < dReturn.length) {
      ...
      throw new Error(
        "INFO:  Difference curves are not supported for ROC curves, as they do not feature consistent x or y values across all curves."
      );
    }
  ...
  const result = matsDataProcessUtils.processDataROC(
    dataset,
    appParams,
    curveInfoParams,
    plotParams,
